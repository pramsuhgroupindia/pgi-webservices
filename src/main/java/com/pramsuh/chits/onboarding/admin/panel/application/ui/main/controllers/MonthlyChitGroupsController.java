...
//    }
    ...
    @PostMapping
    public ResponseEntity<MonthlyChitGroups> createMonthlyChitGroup(@RequestBody MonthlyChitGroups object){
        object.setNow(LocalDateTime.now());
        if(monthlyChitGroupsRepository.findChitGroupByCode(object.getCode()) == null) {
            MonthlyChitGroups monthlyChitGroups1 = monthlyChitGroupsRepository.save(object);
            if(monthlyChitGroups1 != null) {
                ...
                return new ResponseEntity<MonthlyChitGroups>(object, HttpStatus.OK);
            } else{
                return new ResponseEntity<MonthlyChitGroups>(object, HttpStatus.BAD_REQUEST);
            }
        } else {
            return new ResponseEntity<MonthlyChitGroups>(object, HttpStatus.ALREADY_REPORTED);
        }
    }
}
