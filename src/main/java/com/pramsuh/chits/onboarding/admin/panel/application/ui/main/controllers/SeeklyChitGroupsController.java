...
@RestController
@RequestMapping("/api/v1/chit/funds/web/services/auctionable/schemes/durations/seekly/amounts")
public class SeeklyChitGroupsController {
    @Autowired
    SeeklyChitGroupsRepository seeklyChitGroupsRepository;
    @Autowired
...
//    }
    ...
    @GetMapping("{code}")
    public SeeklyChitGroups getSelectedFeeklySchemeDurationAmountGroup(@PathVariable String code) {
        return seeklyChitGroupsRepository.findChitGroupByCode(code);
    }

    @GetMapping
    public List<SeeklyChitGroups> getAllBiDailySchemeDurationAmountGroups() {
        return seeklyChitGroupsRepository.findAll();
    }

    @PostMapping
    public ResponseEntity<SeeklyChitGroups> createBiDailyChitGroup(@RequestBody SeeklyChitGroups object){
        object.setNow(LocalDateTime.now());
        if(seeklyChitGroupsRepository.findChitGroupByCode(object.getCode()) == null) {
            SeeklyChitGroups seeklyChitGroups1 = seeklyChitGroupsRepository.save(object);
            if(seeklyChitGroups1 != null) {
                ...
                return new ResponseEntity<SeeklyChitGroups>(object, HttpStatus.OK);
            } else{
                return new ResponseEntity<SeeklyChitGroups>(object, HttpStatus.BAD_REQUEST);
            }
        } else {
            return new ResponseEntity<SeeklyChitGroups>(object, HttpStatus.ALREADY_REPORTED);
        }
    }
}
