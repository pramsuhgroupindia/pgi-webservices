...
@RestController
@RequestMapping("/api/v1/chit/funds/web/services/auctionable/schemes/durations/tridaily/amounts")
public class TriDailyChitGroupsController {
    @Autowired
    TriDailyChitGroupsRepository triDailyChitGroupsRepository;
    @Autowired
...
//    }
    ...
    @GetMapping("{code}")
    public TriDailyChitGroups getSelectedBiDailySchemeDurationAmountGroup(@PathVariable String code) {
        return triDailyChitGroupsRepository.findChitGroupByCode(code);
    }

    @GetMapping
    public List<TriDailyChitGroups> getAllTriDailySchemeDurationAmountGroups() {
        return triDailyChitGroupsRepository.findAll();
    }

    @PostMapping
    public ResponseEntity<TriDailyChitGroups> createTriDailyChitGroup(@RequestBody TriDailyChitGroups object){
        object.setNow(LocalDateTime.now());
        if(triDailyChitGroupsRepository.findChitGroupByCode(object.getCode()) == null) {
            TriDailyChitGroups triDailyChitGroups1 = triDailyChitGroupsRepository.save(object);
            if(triDailyChitGroups1 != null) {
                ...
                return new ResponseEntity<TriDailyChitGroups>(object, HttpStatus.OK);
            } else{
                return new ResponseEntity<TriDailyChitGroups>(object, HttpStatus.BAD_REQUEST);
            }
        } else {
            return new ResponseEntity<TriDailyChitGroups>(object, HttpStatus.ALREADY_REPORTED);
        }
    }
}
