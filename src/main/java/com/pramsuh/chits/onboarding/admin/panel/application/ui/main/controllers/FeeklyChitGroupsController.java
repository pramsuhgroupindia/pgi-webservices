...
@RestController
@RequestMapping("/api/v1/chit/funds/web/services/auctionable/schemes/durations/feekly/amounts")
public class FeeklyChitGroupsController {
    @Autowired
    FeeklyChitGroupsRepository feeklyChitGroupsRepository;
    @Autowired
...
//    }
    ...
    @GetMapping("{code}")
    public FeeklyChitGroups getSelectedFeeklySchemeDurationAmountGroup(@PathVariable String code) {
        return feeklyChitGroupsRepository.findChitGroupByCode(code);
    }

    @GetMapping
    public List<FeeklyChitGroups> getAllBiDailySchemeDurationAmountGroups() {
        return feeklyChitGroupsRepository.findAll();
    }

    @PostMapping
    public ResponseEntity<FeeklyChitGroups> createBiDailyChitGroup(@RequestBody FeeklyChitGroups object){
        object.setNow(LocalDateTime.now());
        if(feeklyChitGroupsRepository.findChitGroupByCode(object.getCode()) == null) {
            FeeklyChitGroups feeklyChitGroups1 = feeklyChitGroupsRepository.save(object);
            if(feeklyChitGroups1 != null) {
                ...
                return new ResponseEntity<FeeklyChitGroups>(object, HttpStatus.OK);
            } else{
                return new ResponseEntity<FeeklyChitGroups>(object, HttpStatus.BAD_REQUEST);
            }
        } else {
            return new ResponseEntity<FeeklyChitGroups>(object, HttpStatus.ALREADY_REPORTED);
        }
    }
}
